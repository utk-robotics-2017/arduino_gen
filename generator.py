...
class Generator:
    ...
    @type_check
    def add_extra_functions(self) -> str:
        rv = ""
        for appendage in self.appendage_dict.values():
    ...
    @type_check
    def write_shell_scripts(self, path: str, arduino: str) -> None:
        ...
        os.chmod("{0:s}/upload.sh".format(path), 0o777)

        with open("{0:s}/serial.sh".format(path), 'w') as serial_fo:
            serial_fo.write("#!/usr/bin/env bash\n")
            serial_fo.write("picocom /dev/%s -b 115200 --echo\n" % arduino)
        os.chmod("{0:s}/serial.sh".format(path), 0o777)

    @type_check
    def write_core_config_file(self, path: str, arduino: str) -> None:
        core_config = {}
        ...
        with open("{0:s}/{1:s}_core.json".format(path, arduino), 'w') as fo:
            fo.write(json.dumps(core_config, indent=4))
        os.chmod("{0:s}/{1:s}_core.json".format(path, arduino), 0o777)
